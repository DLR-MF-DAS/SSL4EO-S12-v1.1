...
if __name__ == '__main__':
    ...
    #     path="data/SSL4EOS12",  # Streaming or local path
    #     modalities=["S2L2A", "S2L1C", "S1GRD", "LULC", "DEM"], # "S2RGB", "NDVI",
    #     # modalities=["S2L2A"],  # "S2RGB", "NDVI",
    #     urls="data/SSL4EOS12/val/[S2L2A,S2L1C,S1GRD,LULC,DEM]/ssl4eos12_shard_000001.tar",  # S2RGB,NDVI,
    #     # urls="data/SSL4EOS12/val/S2L2A/ssl4eos12_shard_000001.tar",  # S2RGB,NDVI,
    #     split="val",
    #     batch_size=1,
    ...
    val_transform = MultimodalTransforms(
        transforms=A.Compose([  # We use albumentations because of the shared transform between image modalities
            ...
            MultimodalNormalize(
                mean={"image": statistics["mean"]["S2L2A"]},
                std={"image": statistics["std"]["S2L2A"]}
            ),
    ...
    dataloader = DataLoader(dataset, batch_size=None, num_workers=2, persistent_workers=True)

    # Iterate over the dataloader
    for batch in dataloader:
        print("Batch keys:", list(batch.keys()))
        # Batch keys: ["__key__", "__url__", "S2L2A", "S2L1C", "S2RGB", "DEM", "NDVI", "LULC"]
